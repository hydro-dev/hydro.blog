module.exports = (config, lib, db) => {
    const
        Router = require('koa-router'),
        router = new Router({ prefix: config.path || '' }),
        blog = new (require('./blog'))(db),
        md5 = require('./md5');

    router
        .use(async (ctx, next) => {
            ctx.state.theme = config.theme;
            ctx._404 = async info => {
            ...
            ctx.state.blog_title = config.blog_title;
        ...
        .get('hydro_blog_post', '/show/:postid', async ctx => {
            ...
            if (!post) await ctx._404([ctx.params.postid]);
                ...
                post.uname = (await lib.user.get_by_uid(post.uid)).uname;
